...
结果（向量）与同shape的权值向量相乘，然后将买卖策略加入到训练模型中去，以收益作为目标函数，训练模型
"""
...
class LstmModel:
    ...
    def build_graph(self):
        ...
        self.diff_yes_today = tf.reduce_sum(np.square(self.today_real_price - self.predict_today_price), keep_dims=True)
        ...
        self.minimize = tf.train.AdamOptimizer().minimize(self.diff_yes_today + self.profit)
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            for day in range(self.TIME_STEP - 1, self.series_length - self.test_sample_num - 1, 1):
                ...
                how_many_buy_sole = self._session.run(
                    self.how_many_buy_sold,
                    {self.data: self.get_history_data(),
                     self.today_real_price: self.get_today_real_price(),
                     self.yesterday_real_price: self.get_yes_real_price()})
                ...
                if self.index % 100 == 0 and self.log_print_on:
                    print("epoch is %d ,today real price is %f ,today predict price is %f, how_many_buy_sold is %f, diff_yes_today is %f"
                          % (i, predict_today_price, self.get_today_real_price()[0][0], how_many_buy_sole, diff_yes_today[0][0]))
    ...
    def test(self):
        ...
        for day in range(test_index, self.series_length - 1, 1):
            ...
            print("=====> predict price is %f, real price is %f, how_many_buy_sole is %f, profit is %f "
                  % (predict_today_price, real_stock_price, how_many_buy_sole, profit))
