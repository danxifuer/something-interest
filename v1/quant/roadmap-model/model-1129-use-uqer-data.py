...
class LstmModel:
    ...
    def buildGraph(self):
        ...
        self.weight = tf.Variable(tf.truncated_normal([option.hiddenCellNum, 2], dtype=tf.float32), name='weight')
        self.bias = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[1, 2]), name='bias')

        # self.medianValue = tf.nn.relu(tf.matmul(self.lastTime, self.weight) + self.bias)
        #
