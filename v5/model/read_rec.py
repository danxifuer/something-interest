...
def read_and_decode(record_file):
    ...
    features = tf.parse_single_example(
        ...
        features={'label': tf.FixedLenFeature([], tf.int64),
                  'data': tf.FixedLenFeature([cfg.time_step * cfg.idx_num], tf.float32)})
    data_raw = features['data']
    label = features['label']
    data = tf.reshape(data_raw, [cfg.time_step, cfg.idx_num])
    data.set_shape([cfg.time_step, cfg.idx_num])
    if cfg.is_training:
